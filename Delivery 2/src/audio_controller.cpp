...
HCHANNEL sAudioController::play_sound_loop(std::string name) {
    sAudioController *audio_contr = Get(name);
    HCHANNEL channel_audio;

    audio_contr->audio = BASS_SampleLoad( false, name.c_str(), 0, 0, 3, BASS_SAMPLE_LOOP);

    channel_audio = BASS_SampleGetChannel( audio_contr->audio, false );

    BASS_ChannelPlay(channel_audio, FALSE);

    return channel_audio;
}

HCHANNEL sAudioController::play(std::string name) {
    sAudioController *audio_contr = Get(name);
    HCHANNEL channel_audio;

    audio_contr->audio = BASS_SampleLoad( false, name.c_str(), 0, 0, 3, 0);
